...
public abstract class SReportMailerMonthlyHtml {
    ...
    public static String generateReportHtml(final Connection connection, final String reportType, final String mailSubject) throws Exception {
        ...
        */
        ...
        // Consulta asociados de negocio
        Statement statementUsrBas = connection.createStatement();
        String queryUsrBas = "SELECT Usb_ID, Usb_Nombre FROM dba.UsuariosBas";
        ResultSet resultSetUsrBas = statementUsrBas.executeQuery(queryUsrBas);
        
        while (resultSetUsrBas.next()) {
            html += "<hr><h2>" + SLibUtils.textToHtml(resultSetUsrBas.getString(nomUsrBas)) + "</h2>";
            ...
            while (resultSetPro.next()){
                ...
                String sqlxPro = "SELECT Pro_ID, YEAR(Pes_FecHorSeg) as Anio, MONTH(Pes_FecHorSeg) as Mes, SUM(Pes_Neto) as Total "
                    ...
                    + "AND Usb_ID = '" + resultSetUsrBas.getString(idUsrBas) + "' "
